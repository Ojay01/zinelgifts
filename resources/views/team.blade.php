...
<div class="bg-white dark:bg-gray-700 rounded-lg shadow-lg overflow-hidden transform transition duration-300 hover:scale-105">
    <div class="h-64 bg-gray-300 dark:bg-gray-600">
        <!-- Placeholder for member photo -->
        <img src="https://via.placeholder.com/400x400.png?text={{ str_replace(' ', '+', $member['name']) }}" alt="{{ $member['name'] }}" class="w-full h-full object-cover">
    </div>
    <div class="p-6">
        <h3 class="text-xl font-bold text-gray-800 dark:text-yellow-500 mb-2">{{ $member['name'] }}</h3>
        <p class="text-sm text-yellow-600 dark:text-yellow-400 mb-4">{{ $member['role'] }}</p>
        <div class="relative">
            <p class="text-gray-600 dark:text-gray-300 text-sm mb-4 bio-short">{{ Str::limit($member['bio'], 150) }}</p>
            <p class="text-gray-600 dark:text-gray-300 text-sm mb-4 bio-full hidden">{{ $member['bio'] }}</p>
            @if (strlen($member['bio']) > 150)
