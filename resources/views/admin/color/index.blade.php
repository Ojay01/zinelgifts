    ...
    <div class="mt-4 sm:mt-6 container mx-auto px-4">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-t border-slate-700 bg-slate-800/50">
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Color</th>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Name</th>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider hidden md:table-cell">Code</th>
                    ...
                    <tbody class="divide-y divide-slate-700">
                        ...
                        <tr class="hover:bg-slate-700/50 transition-colors">
                            <td class="px-3 sm:px-6 py-2 sm:py-4">
                                <div class="w-8 h-8 rounded border border-slate-600" style="background-color: {{ $color->value }}"></div>
                            </td>
                            ...
                            <td class="px-3 sm:px-6 py-2 sm:py-4">
                                <div class="flex space-x-2">
                                    <button 
                                        onclick="colorManager.openColorModal('edit', {{ json_encode($color) }})"
                                    ...
                                    <button 
                                        onclick="colorManager.openConfirmModal('{{ route('colors.destroy', $color) }}')"
        ...
        >
            <div class="bg-slate-800 rounded-lg p-6 max-w-sm w-full mx-8 md:mx-auto">
                ...
                <form 
                    id="colorForm"
                    data-store-url="{{ route('colors.store') }}"
                    data-update-url="{{ route('colors.update', '__ID__') }}"
                    method="POST"
                >
                    @csrf
                    ...
                    <div class="mb-4">
                        <label for="valueInput" class="block text-sm font-medium text-gray-300 mb-2">Color Code</label>
                        <div class="flex space-x-2">
                            <input 
                                type="color" 
                                name="value" 
                                id="valueInput" 
                                required 
                                class="h-10 bg-slate-800/50 border border-slate-600 rounded"
                            >
                            <input 
                                type="text" 
                                id="hexInput" 
        ...
        >
            <div class="bg-slate-800 rounded-lg p-6 max-w-sm mx-8 md:mx-auto">
                ...
                <p class="mb-4 text-gray-400">Are you sure you want to delete this color? This action cannot be undone.</p>
    ...
    <script>
        class ColorManager {
            ...
            init() {
                ...
                this.hexInput.addEventListener('input', (e) => {
                    if (/^#[0-9A-F]{6}$/i.test(e.target.value)) {
                        this.valueInput.value = e.target.value;
                    }
                });
            }
            ...
            openColorModal(mode, data = null) {
                ...
                if (mode === 'add') {
                    this.modalTitle.textContent = 'Add Color';
                    ...
                    this.modalTitle.textContent = 'Edit Color';
