<x-guest-layout>
    ...
    <div class="bg-gray-100 dark:bg-gray-900 py-16">
        <div class="container mx-auto px-4">
            <h1 class="text-4xl text-center font-bold text-gray-800 dark:text-white mb-4">{{ $subcategory->name }} Products</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($products as $product)
                    <div class="group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl flex flex-col h-full">
                        <div class="relative aspect-w-1 aspect-h-1 overflow-hidden">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-full h-full max-h-80 object-cover transition-transform duration-300 group-hover:scale-110">
                ...
                @empty
                    <p class="text-gray-500 text-center dark:text-gray-400">No products found in this subcategory.</p>
                @endforelse
            </div>
